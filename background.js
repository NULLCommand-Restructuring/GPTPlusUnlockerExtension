...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    if (message.type == 'unlock') {
        ...
        function refetchurl() {
            fetch('https://gptplus-openfxt.vercel.app/unlock_server/second_source.txt', { cache: 'no-cache', headers: { 'Cache-Control': 'no-cache, no-store, must-revalidate' } })
        ...
        fetch('https://gptplus-openfxt.vercel.app/unlock_server/first_source.txt', { cache: 'no-cache', headers: { 'Cache-Control': 'no-cache, no-store, must-revalidate' } })
            .then(response => response.text())
            .then(data => {
                dataObj = extractAndPrintInfo(data)
                fetch(dataObj.url, {
